...
BASE_URL = __settings__.getSetting("main_url")
...
##        ("punjabi", "", "Punjabi"),
    ...
    for lang_item in languages:
        ...
        if "http" not in lang_item[1] and lang_item[1] != "":
            ...
            image = img_path + title + "_Movies.jpg"
...
def get_movie_link(s, mainurl, headers=None):
    import time

    htm=s.get(mainurl, headers=headers, cookies=s.cookies).text
    addLog("HTML Text: " + htm)
    lnk=re.findall('data-ejpingables=["\'](.*?)["\']',htm)[0]

    if lnk == '':
        for i in range(1,10):
            addLog("Retrying to get the LINK after...: " + str(i))
            time.sleep(60 - (15 * (i % 4)))
            htm=s.get(mainurl, headers=headers, cookies=s.cookies).text
            lnk=re.findall('data-ejpingables=["\'](.*?)["\']',htm)[0]
            if lnk:
                break
...
def get_video(s, language, movieid, hdtype, refererurl, oldejp=""):
    ...
    pghtm = get_movie_link(s, videourl, headers)
    lnk = pghtm[0]
    if lnk == '':
        dialog = xbmcgui.Dialog()
        ret = dialog.select('Retry Options', ['Try Again', 'Stop'])
        if ret ==0:
            pghtm = get_movie_link(s, videourl, headers)

    ejp = pghtm[0]
    html1 = pghtm[1]
    addLog("Encoded LINK after retry:" + ejp)
